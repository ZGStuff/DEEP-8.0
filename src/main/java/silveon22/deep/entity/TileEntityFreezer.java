...
import silveon22.deep.block.misc.utils.LookupFuelFreezer;
...
public class TileEntityFreezer extends TileEntity implements Container {
	...
	public @Nullable ItemStack removeItem(int index, int takeAmount) {
		if (this.freezerItemStacks[index] != null) {
			if (this.freezerItemStacks[index].stackSize <= takeAmount) {
				...
				TilePos itemStackBlockPos = new TilePos(this.tilePos.x, this.tilePos.y, this.tilePos.z);
				if (this.worldObj != null && index == 2) {
					this.worldObj.markBlockNeedsUpdate(itemStackBlockPos);
				}
	...
	public void setItem(int index, @Nullable ItemStack itemstack) {
		TilePos setItemBlockPos = new TilePos(this.tilePos.x, this.tilePos.y, this.tilePos.z);
		...
		if (this.worldObj != null && index == 2 && itemstack == null) {
			this.worldObj.markBlockNeedsUpdate(setItemBlockPos);
		}
	...
	public void tick() {
		...
		if (this.worldObj == null || !this.worldObj.isClientSide) {
			if ((this.worldObj == null || this.worldObj.getBlockId(this.tilePos.x, this.tilePos.y, this.tilePos.z) == DEEPBlocks.freezerIdle.id()) && this.currentBurnTime == 0 && this.freezerItemStacks[0] == null && this.freezerItemStacks[1] != null && this.freezerItemStacks[1].itemID == Blocks.PERMAICE.id()) {
			...
			if (this.currentBurnTime == 0 && this.freezerItemStacks[1] != null && this.canSmelt()) {
				...
				if (this.currentBurnTime > 0) {
					...
					if (this.freezerItemStacks[1] != null) {
						if (this.freezerItemStacks[1].getItem() == Items.BUCKET_IRON) {
							this.freezerItemStacks[1] = new ItemStack(Items.BUCKET_IRON);
						} else {
							--this.freezerItemStacks[1].stackSize;
							if (this.freezerItemStacks[1].stackSize <= 0) {
								this.freezerItemStacks[1] = null;
	...
	public void smeltItem() {
		TilePos smeltPos = new TilePos(this.tilePos.x, this.tilePos.y, this.tilePos.z);
		if (this.canSmelt()) {
			List<RecipeEntryFreezer> list = DEEPRecipes.getAllRecipesOfType(RecipeEntryFreezer.class);
			ItemStack itemstack = null;
			...
			if (this.worldObj != null && wasEmpty && this.freezerItemStacks[2] != null) {
				this.worldObj.markBlockNeedsUpdate(smeltPos);
			}
	...
	protected void updateFreezer(boolean forceLit) {
		if (this.worldObj != null) {
			BlockLogicFreezer.updateFreezerBlockState(forceLit | this.currentBurnTime > 0, this.worldObj, this.tilePos.x, this.tilePos.y, this.tilePos.z);
